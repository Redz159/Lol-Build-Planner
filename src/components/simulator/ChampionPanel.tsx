...
interface Props {
  ...
  // Which way the item/rune dropdowns open — 'right' on the right-hand side so they stay on screen.
  align?: 'left' | 'right'
}
  ...
  return (
    <div className="sim-panel" style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {block ? <StatSheet block={block} resourceType={props.resourceType} /> : !props.error && <div style={{ color: 'var(--text-dim)', fontSize: 13 }}>Loading champion data...</div>}
        <ItemsDropdown
          align={align}
          items={items}
